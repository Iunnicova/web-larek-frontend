...
#### Класс AppState
Класс AppState содержит несколько методов, связанных с управлением заказами и каталогом продуктов. Ниже приведено краткое изложение функций, предоставляемых методами в AppStateклассе:

getTotal():

Рассчитывает общую стоимость товаров в заказе путем суммирования цен товаров из каталога на основе их идентификаторов.
setCatalog(items: IOrdering[]): void:

Устанавливает каталог продукции в состояние и генерирует событие изменения.
setPreview(item: IOrdering):

Устанавливает элемент предварительного просмотра в состояние и генерирует событие изменения предварительного просмотра.
getActiv(): IOrdering[]:

Извлекает активные элементы, настроенные для участия в каталоге.
getClosedLots(): IOrdering[]:

Извлекает из каталога закрытые лоты, отмеченные определенным состоянием.
setOrderField(field: keyof IOrderForm, value: string):

Устанавливает поле в объекте заказа и генерирует событие, если заказ проверен.
validateOrder():

Проверяет поля заказа, такие как оплата, адрес, электронная почта и телефон, устанавливает ошибки формы и генерирует событие изменения ошибки формы.
getOrder(): IOrder:

Рассчитывает общую стоимость, обновляет позиции заказа и возвращает объект заказа.
Эти методы совместно обрабатывают операции, связанные с управлением заказами, обновлением каталога, проверкой заказов и извлечением определенных товаров из каталога на основе условий.
...
-`show:products` - Вывод карточек товара на экран
...
- `preview:changed`- предварительный просмотр товара
- '**data:error**' - ошибка ввода данных
- 'catalog:changed' -  отрисовка карточек в галерее;

* 'ui:basket- add' - пользовательский интерфейс:корзина- добавить
* 'ui:basket-remove'- пользовательский интерфейс: корзина
* 'basket:open' - модальное окно корзины открыто
* `basket:change` - изменение данных в корзине, установка новых данных в модель
* 'order:ready' - проверяет заказ - заказ готов
* 'order:open'  - в модальном окне отрисовывается форма заказа
* 'payment:change' - выбранный метод платежа
* 'card:select' - выбор карточки продукта

******AppData
Это метод 'setCatalog' в классе, который обновляет каталог товаров. Он принимает массив товаров items типа IProductCard[], копирует свойства каждого товара и добавляет свойство statusBasket с значением false. Затем он вызывает метод emitChanges с событием 'catalog:changed', чтобы уведомить о изменении каталога.